...
public class SessionLogsAmqpHandler {
	...
	public void createSessionLogs(TaskDescription task) throws IOException, TimeoutException {
		...
		if (count == 0) {
			...
			report = TaskReport.successful(task.getTaskId(), new LinkExchangeModel().getSessionLogsLinks().setSimpleLink(formatLink(RestApi.Cobra.Sessions.GET_SIMPLE, aid, services, links))
					.setExtendedLink(formatLink(RestApi.Cobra.Sessions.GET_EXTENDED, aid, services, links)).parent());
		}
